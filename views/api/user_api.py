...
def attention_project(result, user):
    data = request.get_json()
    attention = Attention.query.filter_by(user_id=user.id, project_id=data['project_id']).first()
    if not attention:
...
def apply_artist(result, user):
    data = request.get_json()
    artist = ArtistProfile.query.filter_by(user_id=user.id).first()
    ...
    if works:
        for work in works:
            work_model = Work(title=work.get('title', ''), actor=work.get('actor', ''),
                              main_actor=work.get('main_actor'), director=work.get('director', ''),
